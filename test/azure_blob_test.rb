...
module Shrine
  module Storage
    class AzureBlobTest < Minitest::Test
      def setup
      ...
      def test_open_raises_shrine_file_not_found_for_missing_blobs
        error = ::AzureBlob::Http::FileNotFoundError.new
        error.stubs(:message).returns("not found")
        @client.expects(:get_blob).with("cache/missing.png", {}).raises(error)
